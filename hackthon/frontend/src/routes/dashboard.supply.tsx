...
function statusBadge(pct: number) {
  if (pct >= 90) return <Badge className="bg-success text-success-foreground hover:bg-success">Full Supply</Badge>;
  if (pct >= 60) return <Badge className="bg-warning text-warning-foreground hover:bg-warning">Partial Supply</Badge>;
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="p-6">
          <div className="mb-3 inline-flex rounded-md bg-muted px-2 py-1 text-xs font-medium">Status</div>
...
function Stat({ title, value, accent }: { title: string; value: string; accent: "primary" | "warning" | "success" }) {
  const cls = accent === "primary" ? "bg-primary/10 text-primary" : accent === "warning" ? "bg-warning/15 text-warning-foreground" : "bg-success/15 text-success";
  return (
    <Card className="p-6">
      <div className={`mb-3 inline-flex rounded-md px-2 py-1 text-xs font-medium ${cls}`}>{title}</div>
