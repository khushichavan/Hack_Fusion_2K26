...
function requestCenter(request: DemandRequest): [number, number] {
  ...
  if (request.location.lat !== undefined && request.location.lng !== undefined)
...
function requestCoordinateLabel(request: DemandRequest) {
  ...
  const hasGps = Boolean(
    request.coordinates ||
    (request.location.lat !== undefined && request.location.lng !== undefined),
  );
...
function UserRequestsPage() {
  ...
  const modalMapInstance = useRef<LeafletMap | null>(null);
  const modalMarkerRef = useRef<Marker | null>(null);
  ...
  const groups = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([area, list]) => ({
        area,
        requests: list,
        totalAmount: list.reduce((sum, req) => sum + req.amount, 0),
      }))
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      marker
        .bindPopup(
          `<strong>${selectedRequest.userName}</strong><br/>Current request location<br/>${selectedRequest.location.area}<br/>${selectedRequest.amount} ML`,
        )
        .openPopup();
  ...
  const update = (id: string, status: RequestStatus, label: string) => {
    ...
    if (r) {
      ...
      notify(
        ...
        `Your ${r.amount} ML request for ${r.location.area} was ${status.toLowerCase()}.`,
        r.userEmail,
      );
    }
  ...
  const combine = (area: string, list: DemandRequest[]) => {
    createCombinedSupplyPlan(
      ...
      list[0]?.location.city,
      list[0]?.location.pincode,
    );
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">
                    {group.requests.length} requests from
                  </p>
                  <h3 className="text-xl font-semibold">{group.area}</h3>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Total demand {group.totalAmount} ML
                  </p>
                </div>
                <Button variant="secondary" onClick={() => combine(group.area, group.requests)}>
                  Create Combined Supply Plan
                </Button>
              </div>
            </Card>
          ))}
            ...
            {combinedPlans.map((plan) => (
              <div key={plan.id} className="rounded-3xl border border-border p-4">
                <div className="text-sm text-muted-foreground">
                  {plan.city ? `${plan.city}, ${plan.pincode}` : "Combined area"}
                </div>
                <div className="mt-2 text-lg font-semibold">{plan.area}</div>
                <p className="text-sm text-muted-foreground">
                  {plan.requestIds.length} requests • {plan.totalAmount} ML
                </p>
              </div>
            ))}
          </div>
                  ...
                  <TableRow key={r.id} className="soft-table-row">
                    ...
                    <TableCell>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {r.location.city} • {r.location.pincode}
                      </div>
                    </TableCell>
                    <TableCell className="text-xs text-muted-foreground">
                      {requestCoordinateLabel(r)}
                    </TableCell>
                    ...
                    <TableCell className="text-right space-y-2">
                      <div className="flex flex-wrap justify-end gap-2">
                        <Button size="sm" variant="outline" onClick={() => viewOnMap(r)}>
                        ...
                        <Button
                          ...
                          variant="outline"
                          disabled={!live}
                          onClick={() => update(r.id, "Approved", "Approved")}
                        >
                          Approve
                        </Button>
                        <Button
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={r.status === "Completed" || r.status === "Expired"}
                          onClick={() => update(r.id, "Completed", "Completed")}
                        >
                          Completed
                        </Button>
                      </div>
      ...
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              View this requester’s location and request details on a map.
            </DialogDescription>
          </DialogHeader>
          {selectedRequest ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    {selectedRequest.amount} ML • {selectedRequest.purpose}
                  </p>
                </div>
              </div>
