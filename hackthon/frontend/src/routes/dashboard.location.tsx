...
function LocationPage() {
  ...
  const [coords, setCoords] = useState<[number, number]>(
    savedLocation?.lat !== undefined && savedLocation?.lng !== undefined ? [savedLocation.lat, savedLocation.lng] : defaultCenter,
  );
  ...
  const lastSavedCoordsRef = useRef<[number, number] | null>(
    savedLocation?.lat !== undefined && savedLocation?.lng !== undefined ? [savedLocation.lat, savedLocation.lng] : null,
  );
  ...
  const useMyLocation = () => {
    ...
    locationWatchRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        const movedEnough = !last || Math.abs(last[0] - c[0]) > 0.00005 || Math.abs(last[1] - c[1]) > 0.00005;
        if (movedEnough) {
          saveGpsLocation(c);
        }
        setIsLocating(false);
      ...
      (error) => {
        setIsLocating(false);
        const message = error.code === error.PERMISSION_DENIED ? "Location permission denied" : "Unable to retrieve location";
      ...
      <Card className="p-6">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"><MapPin className="h-6 w-6" /></div>
            <div>
          ...
          <div className="flex flex-wrap gap-3">
            <Button onClick={useMyLocation} disabled={isLocating}>
              {isLocating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Use My Current Location
            </Button>
            <Button variant="outline" onClick={saveManualLocation}>Save Manual Location</Button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-3xl border border-border bg-muted/40 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2"><Label>Area / Locality</Label><Input value={manual.area} onChange={(e) => setManual({ ...manual, area: e.target.value })} /></div>
              <div className="space-y-2"><Label>City</Label><Input value={manual.city} onChange={(e) => setManual({ ...manual, city: e.target.value })} /></div>
              <div className="space-y-2"><Label>Pincode</Label><Input value={manual.pincode} onChange={(e) => setManual({ ...manual, pincode: e.target.value })} /></div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-border bg-background p-6 shadow-sm">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Saved location</div>
                <div className="text-lg font-semibold mt-2">{hasSaved ? savedLocation?.area : "No saved location"}</div>
              </div>
            </div>
            {hasSaved ? (
              <div className="space-y-2 text-sm text-muted-foreground">
                <div><span className="font-semibold text-foreground">City:</span> {savedLocation?.city}</div>
                <div><span className="font-semibold text-foreground">Pincode:</span> {savedLocation?.pincode}</div>
                <div><span className="font-semibold text-foreground">Coordinates:</span> {savedLocation?.lat?.toFixed(5) ?? "N/A"}, {savedLocation?.lng?.toFixed(5) ?? "N/A"}</div>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">Save a location with GPS or manual entry to enable demand requests.</p>
            )}
          </div>
        </div>
